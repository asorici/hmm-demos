...
while abs(LogP_old - sum(LogP) / L) >= 0.000001 && iter_ct < max_iter
    ...
    %% Expectation
    % Compute Xi
    % you have the values of A, B, Alpha, Beta and Scale
    Xi = zeros(L,T-1,N,N);
% xi_disc-start - Write code below
    for l = 1:L
        for t = 1:(T-1)
            s_ = 0;
            for i = 1:N
...
% xi_disc-end - Write code above
    % Compute Gamma
    % you have the values of A, B, Alpha, Beta, Scale and Xi
    Gamma = zeros(L,T,N);
% gamma_disc-start - Write code below
    for l = 1:L
        for t = 1:(T-1)
            for i = 1:N
                Gamma(l,t,i) = Alpha(l,t,i)*Beta(l,t,i);
            end
            Gamma(l,t,:) = Gamma(l,t,:) ./ exp(logP(l));
        end
    end
    
% gamma_disc-end - Write code above

    %% Maximization (Reestimation)
    % reestimate A, B, Pi
    % you have the old values of A, B, Alpha, Beta, Scale, Gamma and Xi
