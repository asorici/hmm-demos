...
% first try with a _train suffix
filename = strcat(symbol, '_train.mat');

% return empty vectors if no training file exists
if ~exist(filename, 'file')
    % try to find a _all suffix
    filename = strcat(symbol, '_all.mat');
    if ~exist(filename, 'file')
        error('symbol_vq_vectors:config', ... 
          'No _train or _all data file was found for symbol %s.\nHalting codebook computation.', ...
          symbol);
    end
end
